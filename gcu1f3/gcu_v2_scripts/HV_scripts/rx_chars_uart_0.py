...
###### set the baudrate #########
hw.getNode("uart_0.setup.clks").write(2170)
hw.dispatch()
print "baudrate set to 57600"

###### set parity bits #########
hw.getNode("uart_0.setup.parity").write(0x0)   
hw.dispatch()
print "parity set to: no parity"

###### set number of stop bits #########
hw.getNode("uart_0.setup.stop").write(0)  
hw.dispatch()
print "1 stop bit"
...
## 2 = 6 bit word
## 3 = 5 bit word
hw.getNode("uart_0.setup.word").write(0x0)  
hw.dispatch()
print "8 bit word"

###### reset receiver #########
hw.getNode("uart_0.rx_data.rx_reset").write(1)
hw.dispatch()
print "receiver reset done"

###### clear receiver errors #########
hw.getNode("uart_0.rx_data.clear").write(0x3)
hw.dispatch()
print "receiver clear errors done"

###### rx fifo level #########
rx_fifo_fill = hw.getNode("uart_0.fifo.rx_fifo_fill").read()
hw.dispatch()
print "rx fifo fill level: "
print rx_fifo_fill.value()

setup = hw.getNode("uart_0.setup").read()
hw.dispatch()
print "setup: %d" % setup.value()

lgln = hw.getNode("uart_0.fifo.rx_fifo_lgln").read()
hw.dispatch()
...
while True:
    sleep(0.1)
    no_empty = hw.getNode("uart_0.fifo.rx_fifo_not_empty").read()
    rx_fifo_fill = hw.getNode("uart_0.fifo.rx_fifo_fill").read()
    hw.dispatch()
...
#    print "Fifo is filled with=", rx_fifo_fill.value()
    ...
    for i in  range(rx_fifo_fill):
        rx_data = hw.getNode("uart_0.rx_data").read()
        hw.dispatch()
