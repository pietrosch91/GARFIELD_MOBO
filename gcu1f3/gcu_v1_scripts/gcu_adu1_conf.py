...
class GCU():
    ...
    def __init__(self, uhal_connection):
        ...
        try:
            self.hw.getNode("ipbus_fifo_1.ctrl").read()
            self.hw.dispatch()
    ...
    def set_trigger_window(self, window=0x200):
        retval = False
        try:
            self.hw.getNode("l1_cache_1.trigger_window").write(window)
            self.hw.dispatch()
            retval = True
        except:
            print "retry IPBUS transaction"
    ...
    def set_trigger_threshold(self, threshold=0x1000):
        retval = False
        try:
            self.hw.getNode("l1_cache_1.trigger_threshold").write(threshold)
            self.hw.dispatch()
            retval = True
        except:
            print "retry IPBUS transaction"
    ...
    def set_pre_trigger(self, value):
        self.hw.getNode("l1_cache_1.pre_trigger").write(value)
        self.hw.dispatch()
    ...
    def acquire_fifo_lock(self):
        ...
        """
        self.hw.getNode("ipbus_fifo_1.ctrl").write(1)

    def release_fifo_lock(self):
        """
        release the lock on the 2 stage fifo
        """
        self.hw.getNode("ipbus_fifo_1.ctrl").write(0)
    ...
    def get_fifo_count(self):
        ...
        """
        count = self.hw.getNode("ipbus_fifo_1.status.count").read()
        return count
    ...
    def read_from_daq_fifo(self, size):
        ...
        """
        fifo = self.hw.getNode("ipbus_fifo_1.data").readBlock(size)
        return fifo
    ...
    def get_fifo_occupation(self):
        ...
        """
        occupied = self.hw.getNode("ipbus_fifo_1.occupied").read()
        return occupied
    ...
    def get_fifo_control(self):
        ...
        """
        control = self.hw.getNode("ipbus_fifo_1.ctrl").read()
        return control
